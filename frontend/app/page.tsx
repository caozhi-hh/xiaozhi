                    ...
                    <div className="max-w-[85%] sm:max-w-[75%]">
                      ...
                      {!(streaming && i === messages.length - 1) && (
                        <div className={`flex items-center gap-1 mt-1 ml-1 transition-opacity ${mobileActionsIdx === i ? "opacity-100" : "opacity-0 max-sm:opacity-0"} hover:opacity-100 group-hover/msg:opacity-100`}>
                          ...
                          {msg.role === "assistant" && msg.content && <><button onClick={() => handleRegenerate(i)} className="text-xs text-gray-400 hover:text-[var(--accent)] px-1.5 py-0.5 rounded transition-colors cursor-pointer" title="重新生成">🔄</button><button onClick={() => speak(msg.content)} className="text-xs text-gray-400 hover:text-[var(--accent)] px-1.5 py-0.5 rounded transition-colors cursor-pointer" title={speaking ? "停止朗读" : "朗读"}>{speaking ? "⏹" : "🔊"}</button><button onClick={() => { const blob = new Blob([msg.content], {type: "text/plain;charset=utf-8"}); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "xiaozhi.txt"; a.click(); URL.revokeObjectURL(url); }} className="text-xs text-gray-400 hover:text-[var(--accent)] px-1.5 py-0.5 rounded transition-colors cursor-pointer" title="保存为文件">💾</button></>}
